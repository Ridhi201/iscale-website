...
const LoginPage = ({ setCurrentPage, onLoginSuccess }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        // If the user hasn't verified OTP yet, redirect them to OTP page
        if (data.needsVerification) {
          if (onLoginSuccess) onLoginSuccess(null, data.email);
    ...
    <div style={{ minHeight: '90vh', background: 'var(--gradient-hero)', display: 'flex', alignItems: 'center', padding: '60px 0' }}>
      <div className="container about-grid">
        ...
        <div style={{
          ...
          <p style={{ color: '#888', marginBottom: 32, fontSize: 14 }}>Welcome back! Please enter your details.</p>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, flexWrap: 'wrap', gap: 8 }}>
            <div>
              <button style={{ background: 'none', border: 'none', color: '#555', fontSize: 13, cursor: 'pointer', display: 'block', marginBottom: 4 }}
                onClick={() => alert('Password reset requires backend integration. Coming soon!')}>
                Lost Your password?
              </button>
              <button style={{ background: 'none', border: 'none', color: '#555', fontSize: 13, cursor: 'pointer' }}
                onClick={() => alert('Email recovery requires backend integration. Coming soon!')}>
                Lost Your Email?
              </button>
            </div>
          ...
          <button
            ...
            style={{
              ...
              transition: 'all 0.3s', cursor: loading ? 'not-allowed' : 'pointer', border: 'none'
            }}
          >
            {loading
          ...
          <button style={{
            ...
            onClick={() => alert('Google login requires OAuth setup. Coming soon!')}>
